        ...
        try:
            ...
            for season in seasons:
                try:
                    ...
                    logger.info(f"Successfully loaded {len(df)} matches from season 20{season[:2]}-20{season[2:]}")
                    time.sleep(1)  # Be respectful to the server
        ...
        try:
            ...
            with open(summary_path, 'w') as f:
                ...
                if 'home_team' in df.columns and 'away_team' in df.columns:
                    f.write(f"Teams: {len(set(df['home_team'].unique()) | set(df['away_team'].unique()))}\n")
