...
class ModelEvaluator:
        ...
        for class_label in classes:
            if class_label in y_true.values:
                metrics[f'precision_{class_label}'] = precision_score(
                    y_true, y_pred, labels=[class_label], average='macro', zero_division=0
                )
    ...
    def _generate_evaluation_report(self, y_true: pd.Series, y_pred: np.ndarray, metrics: Dict[str, float]) -> None:
    ...
    def _plot_metrics(self, metrics: Dict[str, float], save_dir: Path) -> None:
        ...
        try:
            ...
            bars = plt.bar(main_metrics, values, color=['skyblue', 'lightgreen', 'lightcoral', 'lightyellow'])
