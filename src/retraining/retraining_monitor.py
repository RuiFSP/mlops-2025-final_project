...
class RetrainingMonitor:
    ...
    def evaluate_model_performance(self, predictions_df: pd.DataFrame) -> PerformanceMetrics | None:
        """Evaluate model performance based on recent predictions."""
        if len(predictions_df) < self.config.min_prediction_count:
            logger.warning(
                f"⚠️ Insufficient predictions for evaluation: {len(predictions_df)} < {self.config.min_prediction_count}"
            )
            return None
    ...
    def check_retraining_triggers(self, metrics: PerformanceMetrics) -> dict[str, Any]:
        ...
        if model_info and "accuracy" in model_info:
            ...
            if degradation > self.config.performance_degradation_threshold:
                triggers.append(
                    f"Performance degradation: {degradation:.3f} > {self.config.performance_degradation_threshold}"
                )
        ...
        if consecutive_poor >= self.config.consecutive_poor_performance_limit:
            triggers.append(
                f"Consecutive poor performance: {consecutive_poor} >= {self.config.consecutive_poor_performance_limit}"
            )
    ...
    def trigger_retraining(self, trigger_info: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            result = {"run_id": run_id, "accuracy": accuracy, "model_version": model_version}
        ...
        try:
            ...
            if metrics is None:
                ...
                return {
                    ...
                    "message": "Unable to evaluate model performance",
                }
