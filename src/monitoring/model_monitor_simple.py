...
class SimpleModelMonitor:
    ...
    def calculate_model_performance(self, current_data: pd.DataFrame) -> Dict[str, Any]:
        ...
        if self.target_column not in current_data.columns:
            logger.warning(f"Target column '{self.target_column}' not found in current data")
            return {}
        
        if self.prediction_column not in current_data.columns:
            logger.warning(f"Prediction column '{self.prediction_column}' not found in current data")
            return {}
    ...
    def generate_monitoring_report(self, current_data: pd.DataFrame, 
        ...
        report = {
            ...
            'summary': {
                ...
                'features_with_drift': sum(1 for m in drift_metrics.values() if m.get('is_drift', False)),
        ...
        self._generate_visualizations(current_data, drift_metrics, performance_metrics, output_path, timestamp)
    ...
    def _generate_visualizations(self, current_data: pd.DataFrame, 
    ...
    def check_for_alerts(self, drift_metrics: Dict[str, Any], 
                        performance_threshold: float = 0.4) -> List[str]:
        ...
        drifted_features = [name for name, metrics in drift_metrics.items() 
                          if metrics.get('is_drift', False)]
        
        if drifted_features:
            alerts.append(f"Data drift detected in features: {', '.join(drifted_features)}")
        
        # Check for performance alerts
        if hasattr(self, 'last_performance'):
            if self.last_performance and self.last_performance < performance_threshold:
                alerts.append(f"Model performance below threshold: {self.last_performance:.3f}")
...
def create_model_monitor(reference_data_path: str, 
                        target_column: str = 'result',
                        prediction_column: str = 'predicted_result') -> SimpleModelMonitor:
