...
try:
    ...
    try:
        ...
        def register_flows(): return None
        def run_flow(flow_name): return None
    ...
    try:
        ...
        class SeasonSimulator: pass
        def get_simulator(): return None
        def start_simulation(*args, **kwargs): return False
        def pause_simulation(): return False
        def resume_simulation(): return False
        def stop_simulation(): return False
        def get_simulation_status(): return {"is_running": False}
...
st.set_page_config(
    page_title="Premier League MLOps Dashboard",
    page_icon="⚽",
    layout="wide",
...
class StreamlitDashboard:
    ...
    def _show_mock_prediction_data(self):
        ...
        mock_data = {
            'home_team': ['Arsenal', 'Liverpool', 'Chelsea', 'Man City', 'Tottenham', 
                         'Leicester', 'Everton', 'West Ham', 'Newcastle', 'Aston Villa'],
            'away_team': ['Man United', 'Chelsea', 'Tottenham', 'Liverpool', 'Arsenal',
                         'Everton', 'Leicester', 'Newcastle', 'Aston Villa', 'West Ham'],
            ...
            with self.metrics_storage._get_connection() as conn:
                ...
                try:
                    ...
                    if not df.empty:
                        ...
                        df['profit_loss'] = df.apply(lambda row: 
                            row['payout'] - row['bet_amount'] if row['result'] == 'W' 
                            else -row['bet_amount'] if row['result'] == 'L' 
                            else 0, axis=1)
                        ...
                        recent_bets['payout'] = recent_bets['payout'].apply(lambda x: f"£{x:.2f}" if pd.notna(x) else "Pending")
                        recent_bets['roi'] = recent_bets['roi'].apply(lambda x: f"{x:.1%}" if pd.notna(x) else "Pending")
                        
                        st.dataframe(recent_bets[['bet_date', 'home_team', 'away_team', 'bet_type', 'bet_amount', 'result', 'payout', 'roi']], 
            ...
            if submitted and self.prediction_pipeline:
                try:
                    ...
                    with col1:
                        ...
                        st.metric("Predicted Outcome", outcome_map.get(prediction['prediction'], prediction['prediction']))
                    ...
                    prob_data = {
                        ...
                        'Probability': [prediction['probabilities']['H'], 
    ...
    def render_simulation_dashboard(self):
        ...
        if is_simulation_running:
            ...
            with col2:
                st.metric("Matches Simulated", f"{sim_status.get('matches_simulated', 0)} / {sim_status.get('total_matches', 380)}")
        ...
        with st.form("simulation_form"):
            ...
            with col2:
                simulation_speed = st.slider("Simulation speed", 1, 10, 5, 
                                          help="Higher values = faster simulation (fewer days per step)")
                end_date = st.date_input("End date", datetime.now().date())
            
            # Betting strategy
            st.subheader("💰 Betting Strategy")
            betting_strategy = st.selectbox("Betting strategy", 
                                         ["Confidence-based", "Kelly Criterion", "Fixed Amount", "No Betting"])
            ...
            col1, col2 = st.columns(2)
            with col1:
                show_metrics = st.checkbox("Show metrics charts", value=True)
                show_predictions = st.checkbox("Show prediction results", value=True)
            
            with col2:
                show_betting = st.checkbox("Show betting performance", value=True)
        ...
        with col2:
            if st.button("▶️ Resume Simulation", disabled=not is_simulation_running or not sim_status.get("is_paused", False)):
...
def main():
    ...
    st.sidebar.info("If you're experiencing issues, run the troubleshooting script: `python scripts/dashboard_troubleshoot.py`")
