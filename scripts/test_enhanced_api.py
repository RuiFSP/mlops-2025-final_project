...
def test_enhanced_api():
    ...
    for i, match in enumerate(test_matches, 1):
        ...
        print(f"  Betting odds: H={match['home_odds']:.2f}, D={match['draw_odds']:.2f}, A={match['away_odds']:.2f}")
        print(f"  Bookmaker margin: {margin:.2f}%")
        print(f"  Odds probabilities (adjusted): H={home_odds_prob:.3f}, D={draw_odds_prob:.3f}, A={away_odds_prob:.3f}")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"    Home: Model={model_home:.3f} vs Odds={home_odds_prob:.3f} (diff: {model_home-home_odds_prob:+.3f})")
                print(f"    Draw: Model={model_draw:.3f} vs Odds={draw_odds_prob:.3f} (diff: {model_draw-draw_odds_prob:+.3f})")
                print(f"    Away: Model={model_away:.3f} vs Odds={away_odds_prob:.3f} (diff: {model_away-away_odds_prob:+.3f})")
